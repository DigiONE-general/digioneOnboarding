...
```
```{r cdm_coverage, echo = FALSE}


render_table(
 coverage_row,
 "MVP result",
 c(" " = 1, "Result of MEDOC Minimum viable product (MVP)" = ncol(coverage_row) - 1)
)
...
render_table(
 ...
 "Table 3.1 : Condition occurrence",
...
render_table(
 ...
 "Table 3.3 : Measurement table summary",
...
render_table(
 cdm_snapshot_obs,
 "Table 4.1 : Observation table summary",
...
render_table(
 mappingCompleteness,
 "Table 5.1 : Vocabulary mapping completedness",
...
```{r medoc_to_omop, echo = FALSE}
render_table(
  medoc_concept_table,
  "Table 6.1 : MEDOC concept has valid concept id and locations",
...
render_table(
  episode_table,
  "Table 6.2 : Episode table checks",
...
## Cancer diagnosis
```{r cancer_pheno, echo = FALSE}
render_table(
  primary_snap_sliced,
  "Table 7.1 : Primary diagnosis -  concept ids",
  c(" " = 1, "Diagnosis by number of patients and domain (full list as csv in results folder)" = ncol(primary_snap_sliced) - 1)
)

```

## Metastasis diagnosis
```{r mets_diag, echo = FALSE}
render_table(
  mets_snapshot_meas,
  "Table 7.2 : Metastasis diagnosis - Measurement concept ids",
  c(" " = 1, "Metastasis diagnosis by number of patients and domain (full list as csv in results folder)" = ncol(mets_snapshot_meas) - 1)
)

```

```{r mets_diag_cond, echo = FALSE}
render_table(
  mets_snap_sliced_condition,
  "Table 7.3 : Metastasis diagnosis - Condition Occurrence concept ids",
...
# Genomic check 
```{r genomic_check, echo = FALSE}
render_table(
  gene_snap_sliced,
  "Table 8.1 : Genomic readiness - genes of interest present",
  c(" " = 1, "Genes of interest by number of patients and domain (full list as csv in results folder)" = ncol(gene_snap_sliced) - 1)
)
...
render_table(
 ...
 "Table 9.1 : Immunotherapy drug completedness",
...
render_table(
  summary_chemo_drugs,
  "Table 9.2 : Chemotherapy drug completedness",
...
render_table(
  ...
  "Table 9.3 : Targeted therapy drug completedness",
...
render_table(
  ...
  "Table 9.4 : Radiotherapy completedness",
...
render_table(
  summary_procedure,
  "Table 10.1 : All procedures check",
...
# TNM check
```{r tnm_check, echo = FALSE}
render_table(
 tnm_result,
 "Table 11.1 : TNM code check",
